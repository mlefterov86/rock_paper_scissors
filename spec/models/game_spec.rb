...
RSpec.describe Game do
  ...
  describe '#play' do
    it 'returns :player when player wins' do
      allow_any_instance_of(described_class).to receive(:computer_choice).and_return(:scissors)
      expect(game.play).to eq(:player)
    end

    it 'returns :computer when computer wins' do
      allow_any_instance_of(described_class).to receive(:computer_choice).and_return(:paper)
      expect(game.play).to eq(:computer)
    end

    it "returns :tie when it's a tie" do
      allow_any_instance_of(described_class).to receive(:computer_choice).and_return(:rock)
